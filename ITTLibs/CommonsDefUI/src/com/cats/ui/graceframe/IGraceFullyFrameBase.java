...
public abstract class IGraceFullyFrameBase extends JFrame implements IGraceFulTitlePanelHandler, IGraceFulIconInfoProvider
{
    ...
    protected Point originLocation = null;
    protected Dimension originSize = null;
    ...
    {
		int i = this.getExtendedState() & Frame.MAXIMIZED_BOTH;
		if(i > 0)//Normal Size
		{
			if(null != this.originLocation && null != this.originSize)
			{
				this.setSize(originSize);
				this.setLocation(originLocation);
			}
		}else //MaxFrame
		{
			originLocation = this.getLocation();
			originSize = this.getSize();
			this.setExtendedState(Frame.MAXIMIZED_BOTH | getExtendedState());
		}
    }
