 ...
 * */
public class CrashMonitorForAndroid6 implements ICrashMonitor
{
	...
	public CrashMonitorForAndroid6(String deviceSerialNo)
	{
			...
			public void run()
			{
				...
				if (deamon != null)
				{
					...
					while (running)
					{
						...
						if (line != null)
						{
							//ILog.getLog().info("debug_temp", line);
							...
							} /*else if(line.contains("I ActivityManager") && line.contains("Process ") &&
							  line.contains(" has died"))
							{
								ILog.getLog().info(serialNo, "Process died with AM: " + line);
								String packageName = line.substring(
								        line.indexOf("Process ") + 8,
								        line.lastIndexOf('('));
								packageName = packageName.trim();
								synchronized (mCallbacks)
								{
									for (CrashCallback cb : mCallbacks)
									{
										cb.onProcessDied(packageName, serialNo, "Process died-AM.");
									}
								}
							}*/
						}
